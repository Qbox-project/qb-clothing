...
local cam = -1
local headingToCam = GetEntityHeading(cache.ped)
local camOffset = 2
...
CreateThread(function()
    for k, _ in pairs(Config.Stores) do
        ...
        if Config.Stores[k].shopType == "surgeon" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentSubstringPlayerName("Surgeon")
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.ClothingRooms) do
            lib.zones.box({
                ...
                rotation = 0,
                onEnter = function(_)
                    local clothingData = Config.ClothingRooms[k]
                    local job = clothingData.isGang and PlayerData.gang.name or PlayerData.job.name

                    if job == clothingData.requiredJob then
                        zoneName = k
                        inZone = true
...
RegisterNUICallback('rotateRight', function(_, cb)
    ...
    local cx, cy = GetPositionByRelativeHeading(cache.ped, heading, camOffset)
...
function GetMaxValues()
    ...
    for k, v in pairs(clothingCategorys) do
        if v.type == "variation" then
            maxModelValues[k].item = GetNumberOfPedDrawableVariations(cache.ped, v.id)
            maxModelValues[k].texture = GetNumberOfPedTextureVariations(cache.ped, v.id, GetPedDrawableVariation(cache.ped, v.id)) -1
        end
        ...
        if v.type == "mask" then
            maxModelValues[k].item = GetNumberOfPedDrawableVariations(cache.ped, v.id)
            maxModelValues[k].texture = GetNumberOfPedTextureVariations(cache.ped, v.id, GetPedDrawableVariation(cache.ped, v.id))
        end
        ...
        if v.type == "prop" then
            maxModelValues[k].item = GetNumberOfPedPropDrawableVariations(cache.ped, v.id)
            maxModelValues[k].texture = GetNumberOfPedPropTextureVariations(cache.ped, v.id, GetPedPropIndex(cache.ped, v.id))
        end
...
function enableCam()
    -- Camera
    local coords = GetOffsetFromEntityInWorldCoords(cache.ped, 0, 2.0, 0)
    ...
    headingToCam = GetEntityHeading(cache.ped) + 90
    camOffset = 2.0
end
...
RegisterNUICallback('setupCam', function(data, cb)
    ...
    if value == 1 then
        camOffset = 0.75
        local cx, cy = GetPositionByRelativeHeading(cache.ped, headingToCam, camOffset)
...
function resetClothing(data)
    -- Face
    SetPedHeadBlendData(cache.ped, data["face"].item, data["face2"].item, nil, data["face"].texture, data["face2"].texture, nil, data["facemix"].shapeMix, data["facemix"].skinMix, nil, true)

    -- Pants
    SetPedComponentVariation(cache.ped, 4, data["pants"].item, 0, 0)
    SetPedComponentVariation(cache.ped, 4, data["pants"].item, data["pants"].texture, 0)

    -- Hair
    SetPedComponentVariation(cache.ped, 2, data["hair"].item, 0, 0)
    SetPedHairColor(cache.ped, data["hair"].texture, data["hair"].texture)

    -- Eyebrows
    SetPedHeadOverlay(cache.ped, 2, data["eyebrows"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 2, 1, data["eyebrows"].texture, 0)

    -- Beard
    SetPedHeadOverlay(cache.ped, 1, data["beard"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 1, 1, data["beard"].texture, 0)

    -- Blush
    SetPedHeadOverlay(cache.ped, 5, data["blush"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 5, 1, data["blush"].texture, 0)

    -- Lipstick
    SetPedHeadOverlay(cache.ped, 8, data["lipstick"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 8, 1, data["lipstick"].item, 0)

    -- Makeup
    SetPedHeadOverlay(cache.ped, 4, data["makeup"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 4, 1, data["makeup"].texture, 0)

    -- Ageing
    SetPedHeadOverlay(cache.ped, 3, data["ageing"].item, 1.0)
    SetPedHeadOverlayColor(cache.ped, 3, 1, data["ageing"].texture, 0)

    -- Arms
    SetPedComponentVariation(cache.ped, 3, data["arms"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 3, data["arms"].item, data["arms"].texture, 0)

    -- T-Shirt
    SetPedComponentVariation(cache.ped, 8, data["t-shirt"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 8, data["t-shirt"].item, data["t-shirt"].texture, 0)

    -- Vest
    SetPedComponentVariation(cache.ped, 9, data["vest"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 9, data["vest"].item, data["vest"].texture, 0)

    -- Torso 2
    SetPedComponentVariation(cache.ped, 11, data["torso2"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 11, data["torso2"].item, data["torso2"].texture, 0)

    -- Shoes
    SetPedComponentVariation(cache.ped, 6, data["shoes"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 6, data["shoes"].item, data["shoes"].texture, 0)

    -- Mask
    SetPedComponentVariation(cache.ped, 1, data["mask"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 1, data["mask"].item, data["mask"].texture, 0)

    -- Badge
    SetPedComponentVariation(cache.ped, 10, data["decals"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 10, data["decals"].item, data["decals"].texture, 0)

    -- Accessory
    SetPedComponentVariation(cache.ped, 7, data["accessory"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 7, data["accessory"].item, data["accessory"].texture, 0)

    -- Bag
    SetPedComponentVariation(cache.ped, 5, data["bag"].item, 0, 2)
    SetPedComponentVariation(cache.ped, 5, data["bag"].item, data["bag"].texture, 0)
    SetPedEyeColor(cache.ped, data['eye_color'].item)
    SetPedHeadOverlay(cache.ped, 9, data['moles'].item, data['moles'].texture)
    SetPedFaceFeature(cache.ped, 0, data['nose_0'].item)
    SetPedFaceFeature(cache.ped, 1, data['nose_1'].item)
    SetPedFaceFeature(cache.ped, 2, data['nose_2'].item)
    SetPedFaceFeature(cache.ped, 3, data['nose_3'].item)
    SetPedFaceFeature(cache.ped, 4, data['nose_4'].item)
    SetPedFaceFeature(cache.ped, 5, data['nose_5'].item)
    SetPedFaceFeature(cache.ped, 6, data['eyebrown_high'].item)
    SetPedFaceFeature(cache.ped, 7, data['eyebrown_forward'].item)
    SetPedFaceFeature(cache.ped, 8, data['cheek_1'].item)
    SetPedFaceFeature(cache.ped, 9, data['cheek_2'].item)
    SetPedFaceFeature(cache.ped, 10, data['cheek_3'].item)
    SetPedFaceFeature(cache.ped, 11, data['eye_opening'].item)
    SetPedFaceFeature(cache.ped, 12, data['lips_thickness'].item)
    SetPedFaceFeature(cache.ped, 13, data['jaw_bone_width'].item)
    SetPedFaceFeature(cache.ped, 14, data['jaw_bone_back_lenght'].item)
    SetPedFaceFeature(cache.ped, 15, data['chimp_bone_lowering'].item)
    SetPedFaceFeature(cache.ped, 16, data['chimp_bone_lenght'].item)
    SetPedFaceFeature(cache.ped, 17, data['chimp_bone_width'].item)
    SetPedFaceFeature(cache.ped, 18, data['chimp_hole'].item)
    SetPedFaceFeature(cache.ped, 19, data['neck_thikness'].item)
    ...
    if data["watch"].item ~= -1 and data["watch"].item ~= 0 then
        SetPedPropIndex(cache.ped, 6, data["watch"].item, data["watch"].texture, true)
    else
    ...
    if data["bracelet"].item ~= -1 and data["bracelet"].item ~= 0 then
        SetPedPropIndex(cache.ped, 7, data["bracelet"].item, data["bracelet"].texture, true)
    else
...
function ChangeVariation(data)
    ...
    if clothingCategory == "pants" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 4, item, 0, 0)
            skinData["pants"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 4)
            SetPedComponentVariation(cache.ped, 4, curItem, item, 0)
            skinData["pants"].texture = item
        end
    elseif clothingCategory == "face" then
        if type == "item" then
            SetPedHeadBlendData(cache.ped, tonumber(item), skinData["face2"].item, nil, skinData["face"].texture, skinData["face2"].texture, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix, nil, true)
            skinData["face"].item = item
        elseif type == "texture" then
            SetPedHeadBlendData(cache.ped, skinData["face"].item, skinData["face2"].item, nil, item, skinData["face2"].texture, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix, nil, true)
            skinData["face"].texture = item
        end
    elseif clothingCategory == "face2" then
        if type == "item" then
            SetPedHeadBlendData(cache.ped, skinData["face"].item, tonumber(item), nil, skinData["face"].texture, skinData["face2"].texture, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix , nil, true)
            skinData["face2"].item = item
        elseif type == "texture" then
            SetPedHeadBlendData(cache.ped, skinData["face"].item, skinData["face2"].item, nil, skinData["face"].texture, item, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix , nil, true)
            skinData["face2"].texture = item
        end
    elseif clothingCategory == "facemix" then
        if type == "skinMix" then
            SetPedHeadBlendData(cache.ped, skinData["face"].item, skinData["face2"].item, nil, skinData["face"].texture, skinData["face2"].texture, nil, skinData["facemix"].shapeMix, item, nil, true)
            skinData["facemix"].skinMix = item
        elseif type == "shapeMix" then
            SetPedHeadBlendData(cache.ped, skinData["face"].item, skinData["face2"].item, nil, skinData["face"].texture, skinData["face2"].texture, nil, item, skinData["facemix"].skinMix , nil, true)
            skinData["facemix"].shapeMix = item
        end
    elseif clothingCategory == "hair" then
        SetPedHeadBlendData(cache.ped, skinData["face"].item, skinData["face2"].item, nil, skinData["face"].texture, skinData["face2"].texture, nil, skinData["facemix"].shapeMix, skinData["facemix"].skinMix , nil, true)
        if type == "item" then
            SetPedComponentVariation(cache.ped, 2, item, 0, 0)
            skinData["hair"].item = item
        elseif type == "texture" then
            SetPedHairColor(cache.ped, item, item)
            skinData["hair"].texture = item
        end
    elseif clothingCategory == "eyebrows" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 2, item, 1.0)
            skinData["eyebrows"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 2, 1, item, 0)
            skinData["eyebrows"].texture = item
        end
    elseif clothingCategory == "beard" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 1, item, 1.0)
            skinData["beard"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 1, 1, item, 0)
            skinData["beard"].texture = item
        end
    elseif clothingCategory == "blush" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 5, item, 1.0)
            skinData["blush"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 5, 1, item, 0)
            skinData["blush"].texture = item
        end
    elseif clothingCategory == "lipstick" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 8, item, 1.0)
            skinData["lipstick"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 8, 1, item, 0)
            skinData["lipstick"].texture = item
        end
    elseif clothingCategory == "makeup" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 4, item, 1.0)
            skinData["makeup"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 4, 1, item, 0)
            skinData["makeup"].texture = item
        end
    elseif clothingCategory == "ageing" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 3, item, 1.0)
            skinData["ageing"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(cache.ped, 3, 1, item, 0)
            skinData["ageing"].texture = item
        end
    elseif clothingCategory == "arms" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 3, item, 0, 2)
            skinData["arms"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 3)
            SetPedComponentVariation(cache.ped, 3, curItem, item, 0)
            skinData["arms"].texture = item
        end
    elseif clothingCategory == "eye_color" then
        if type == "item" then
            SetPedEyeColor(cache.ped, item)
            skinData["eye_color"].item = item
        end
    elseif clothingCategory == "moles" then
        if type == "item" then
            SetPedHeadOverlay(cache.ped, 9, item, 1.0)
            skinData["moles"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 9)
            local newitem = (item / 10)
            SetPedHeadOverlayColor(cache.ped, 9, curItem, newitem)
            skinData["moles"].texture = item
        end
    elseif clothingCategory == "nose_0" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 0, newitem)
            skinData["nose_0"].item = item
        end
    elseif clothingCategory == "nose_1" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 1, newitem)
            skinData["nose_1"].item = item
        end
    elseif clothingCategory == "nose_2" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 2, newitem)
            skinData["nose_2"].item = item
        end
    elseif clothingCategory == "nose_3" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 3, newitem)
            skinData["nose_3"].item = item
        end
    elseif clothingCategory == "nose_4" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 4, newitem)
            skinData["nose_4"].item = item
        end
    elseif clothingCategory == "nose_5" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 5, newitem)
            skinData["nose_5"].item = item
        end
    elseif clothingCategory == "eyebrown_high" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 6, newitem)
            skinData["eyebrown_high"].item = item
        end
    elseif clothingCategory == "eyebrown_forward" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 7, newitem)
            skinData["eyebrown_forward"].item = item
        end
    elseif clothingCategory == "cheek_1" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 8, newitem)
            skinData["cheek_1"].item = item
        end
    elseif clothingCategory == "cheek_2" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 9, newitem)
            skinData["cheek_1"].item = item
        end
    elseif clothingCategory == "cheek_3" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 10, newitem)
            skinData["cheek_3"].item = item
        end
    elseif clothingCategory == "eye_opening" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 11, newitem)
            skinData["eye_opening"].item = item
        end
    elseif clothingCategory == "lips_thickness" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 12, newitem)
            skinData["lips_thickness"].item = item
        end
    elseif clothingCategory == "jaw_bone_width" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 13, newitem)
            skinData["jaw_bone_width"].item = item
        end
    elseif clothingCategory == "jaw_bone_back_lenght" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 14, newitem)
            skinData["jaw_bone_back_lenght"].item = item
        end
    elseif clothingCategory == "chimp_bone_lowering" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 15, newitem)
            skinData["chimp_bone_lowering"].item = item
        end
    elseif clothingCategory == "chimp_bone_lenght" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 16, newitem)
            skinData["chimp_bone_lenght"].item = item
        end
    elseif clothingCategory == "chimp_bone_width" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 17, newitem)
            skinData["chimp_bone_width"].item = item
        end
    elseif clothingCategory == "chimp_hole" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 18, newitem)
            skinData["chimp_hole"].item = item
        end
    elseif clothingCategory == "neck_thikness" then
        if type == "item" then
            local newitem = (item / 10)
            SetPedFaceFeature(cache.ped, 19, newitem)
            skinData["chimp_hole"].item = item
        end
    elseif clothingCategory == "t-shirt" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 8, item, 0, 2)
            skinData["t-shirt"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 8)
            SetPedComponentVariation(cache.ped, 8, curItem, item, 0)
            skinData["t-shirt"].texture = item
        end
    elseif clothingCategory == "vest" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 9, item, 0, 2)
            skinData["vest"].item = item
        elseif type == "texture" then
            SetPedComponentVariation(cache.ped, 9, skinData["vest"].item, item, 0)
            skinData["vest"].texture = item
        end
    elseif clothingCategory == "bag" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 5, item, 0, 2)
            skinData["bag"].item = item
        elseif type == "texture" then
            SetPedComponentVariation(cache.ped, 5, skinData["bag"].item, item, 0)
            skinData["bag"].texture = item
        end
    elseif clothingCategory == "decals" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 10, item, 0, 2)
            skinData["decals"].item = item
        elseif type == "texture" then
            SetPedComponentVariation(cache.ped, 10, skinData["decals"].item, item, 0)
            skinData["decals"].texture = item
        end
    elseif clothingCategory == "accessory" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 7, item, 0, 2)
            skinData["accessory"].item = item
        elseif type == "texture" then
            SetPedComponentVariation(cache.ped, 7, skinData["accessory"].item, item, 0)
            skinData["accessory"].texture = item
        end
    elseif clothingCategory == "torso2" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 11, item, 0, 2)
            skinData["torso2"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 11)
            SetPedComponentVariation(cache.ped, 11, curItem, item, 0)
            skinData["torso2"].texture = item
        end
    elseif clothingCategory == "shoes" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 6, item, 0, 2)
            skinData["shoes"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 6)
            SetPedComponentVariation(cache.ped, 6, curItem, item, 0)
            skinData["shoes"].texture = item
        end
    elseif clothingCategory == "mask" then
        if type == "item" then
            SetPedComponentVariation(cache.ped, 1, item, 0, 2)
            skinData["mask"].item = item
        elseif type == "texture" then
            local curItem = GetPedDrawableVariation(cache.ped, 1)
            SetPedComponentVariation(cache.ped, 1, curItem, item, 0)
            skinData["mask"].texture = item
        end
    elseif clothingCategory == "hat" then
        if type == "item" then
            if item ~= -1 then
                SetPedPropIndex(cache.ped, 0, item, skinData["hat"].texture, true)
            else
                ClearPedProp(cache.ped, 0)
            end
        ...
        elseif type == "texture" then
            SetPedPropIndex(cache.ped, 0, skinData["hat"].item, item, true)
    ...
    elseif clothingCategory == "glass" then
        if type == "item" then
            if item ~= -1 then
                SetPedPropIndex(cache.ped, 1, item, skinData["glass"].texture, true)
                skinData["glass"].item = item
            else
                ClearPedProp(cache.ped, 1)
            end
        elseif type == "texture" then
            SetPedPropIndex(cache.ped, 1, skinData["glass"].item, item, true)
    ...
    elseif clothingCategory == "ear" then
        if type == "item" then
            if item ~= -1 then
                SetPedPropIndex(cache.ped, 2, item, skinData["ear"].texture, true)
            else
                ClearPedProp(cache.ped, 2)
            end
        ...
        elseif type == "texture" then
            SetPedPropIndex(cache.ped, 2, skinData["ear"].item, item, true)
    ...
    elseif clothingCategory == "watch" then
        if type == "item" then
            if item ~= -1 then
                SetPedPropIndex(cache.ped, 6, item, skinData["watch"].texture, true)
            else
                ClearPedProp(cache.ped, 6)
            end
        ...
        elseif type == "texture" then
            SetPedPropIndex(cache.ped, 6, skinData["watch"].item, item, true)
    ...
    elseif clothingCategory == "bracelet" then
        if type == "item" then
            if item ~= -1 then
                SetPedPropIndex(cache.ped, 7, item, skinData["bracelet"].texture, true)
            else
                ClearPedProp(cache.ped, 7)
            end
        ...
        elseif type == "texture" then
            SetPedPropIndex(cache.ped, 7, skinData["bracelet"].item, item, true)
...
RegisterNetEvent("qb-clothes:loadSkin", function(_, model, data)
    ...
    CreateThread(function()
        ...
        TriggerEvent('qb-clothing:client:loadPlayerClothing', data, cache.ped)
    end)
end)
...
RegisterNetEvent('qb-clothing:client:loadOutfit', function(oData)
    ...
    if data["accessory"] then
        ...
        if QBCore.Functions.GetPlayerData().metadata["tracker"] then
            ...
            SetPedComponentVariation(cache.ped, 7, -1, 0, 2)
        end
    end
...
RegisterNetEvent("qb-clothing:client:adjustfacewear", function(type)
    ...
    faceProps[6]["Prop"] = GetPedDrawableVariation(cache.ped, 0)
    faceProps[6]["Palette"] = GetPedPaletteVariation(cache.ped, 0)
    faceProps[6]["Texture"] = GetPedTextureVariation(cache.ped, 0)

    for i = 0, 3 do
        if GetPedPropIndex(cache.ped, i) ~= -1 then
            faceProps[i + 1]["Prop"] = GetPedPropIndex(cache.ped, i)
        end
        if GetPedPropTextureIndex(cache.ped, i) ~= -1 then
            faceProps[i + 1]["Texture"] = GetPedPropTextureIndex(cache.ped, i)
        end
    end

    if GetPedDrawableVariation(cache.ped, 1) ~= -1 then
        faceProps[4]["Prop"] = GetPedDrawableVariation(cache.ped, 1)
        faceProps[4]["Palette"] = GetPedPaletteVariation(cache.ped, 1)
        faceProps[4]["Texture"] = GetPedTextureVariation(cache.ped, 1)
    end

    if GetPedDrawableVariation(cache.ped, 11) ~= -1 then
        faceProps[5]["Prop"] = GetPedDrawableVariation(cache.ped, 11)
        faceProps[5]["Palette"] = GetPedPaletteVariation(cache.ped, 11)
        faceProps[5]["Texture"] = GetPedTextureVariation(cache.ped, 11)
    end
    ...
    lib.requestAnimDict(AnimSet)

    if type == 5 then
        if removeWear then
            SetPedComponentVariation(cache.ped, 3, 2, faceProps[6]["Texture"], faceProps[6]["Palette"])
        end
    end

    if removeWear then
        TaskPlayAnim(cache.ped, AnimSet, AnimationOff, 4.0, 3.0, -1, 49, 1.0, 0, 0, 0)
        RemoveAnimDict(AnimSet)

        Wait(500)

        if type ~= 5 then
            if type == 4 then
                SetPedComponentVariation(cache.ped, PropIndex, -1, -1, -1)
            else
                if type ~= 2 then
                    ClearPedProp(cache.ped, tonumber(PropIndex))
                end
            end
        end
    else
        TaskPlayAnim(cache.ped, AnimSet, AnimationOn, 4.0, 3.0, -1, 49, 1.0, 0, 0, 0)
        RemoveAnimDict(AnimSet)
        ...
        if type ~= 5 and type ~= 2 then
            if type == 4 then
                SetPedComponentVariation(cache.ped, PropIndex, faceProps[type]["Prop"], faceProps[type]["Texture"], faceProps[type]["Palette"])
            else
                SetPedPropIndex(cache.ped, tonumber(PropIndex), tonumber(faceProps[PropIndex + 1]["Prop"]), tonumber(faceProps[PropIndex + 1]["Texture"]), false)
            end
        end
    end
    ...
    if type == 5 then
        if not removeWear then
            SetPedComponentVariation(cache.ped, 3, 1, faceProps[6]["Texture"], faceProps[6]["Palette"])
            SetPedComponentVariation(cache.ped, PropIndex, faceProps[type]["Prop"], faceProps[type]["Texture"], faceProps[type]["Palette"])
        else
            SetPedComponentVariation(cache.ped, PropIndex, -1, -1, -1)
        end
...
function reloadSkin(health)
    ...
    if gender == 1 then -- Gender is ONE for FEMALE
        model = joaat("mp_f_freemode_01") -- Female Model
    else
        model = joaat("mp_m_freemode_01") -- Male Model
    end
    ...
    SetPedMaxHealth(cache.playerId, maxhealth)
